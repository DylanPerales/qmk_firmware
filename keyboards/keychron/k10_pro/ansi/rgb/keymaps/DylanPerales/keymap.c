...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [WIN_BASE] = LAYOUT_ansi_108(
        KC_ESC,             KC_F1,    KC_F2,    KC_F3,    KC_F4,    KC_F5,    KC_F6,    KC_F7,    KC_F8,    KC_F9,    KC_F10,   KC_F11,     KC_F12,   KC_PSCR,  KC_CTANA, RGB_MOD,  KC_CRCL,  KC_TRI,   KC_SQR,   KC_XXX,
        KC_GRV,   KC_1,     KC_2,     KC_3,     KC_4,     KC_5,     KC_6,     KC_7,     KC_8,     KC_9,     KC_0,     KC_MINS,  KC_EQL,     KC_BSPC,  KC_INS,   KC_HOME,  KC_PGUP,  KC_NUM,   KC_PSLS,  KC_PAST,  KC_PMNS,
        KC_TAB,   KC_Q,     KC_W,     KC_E,     KC_R,     KC_T,     KC_Y,     KC_U,     KC_I,     KC_O,     KC_P,     KC_LBRC,  KC_RBRC,    KC_BSLS,  KC_DEL,   KC_END,   KC_PGDN,  KC_P7,    KC_P8,    KC_P9,    KC_PPLS,
        KC_CAPS,  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,              KC_ENT,                                 KC_P4,    KC_P5,    KC_P6,
        KC_LSFT,            KC_Z,     KC_X,     KC_C,     KC_V,     KC_B,     KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,              KC_RSFT,            KC_UP,              KC_P1,    KC_P2,    KC_P3,    KC_PENT,
...
bool process_record_user(uint16_t keycode, keyrecord_t* record) {
    switch (keycode) {
        case KC_CTANA: // MIC|CORTANA BUTTON
            if (get_mods() == MOD_BIT(KC_LCTL)) { // CTRL MODIFIER
                if (record->event.pressed) {
                    // RECOILLESS RIFLE
                    tap_code_delay(KC_DOWN, 50);
                    tap_code_delay(KC_LEFT, 50);
                    tap_code_delay(KC_RGHT, 50);
                    tap_code_delay(KC_P, 50);
                    tap_code_delay(KC_RGHT, 50);
                    tap_code_delay(KC_LEFT, 50);
                    return false;
                }
            }
            if (get_mods() & MOD_MASK_CA) { // CTRL + ALT MODIFIER
                if (record->event.pressed) {
                    // RAILGUN 
                    tap_code_delay(KC_DOWN, 50);
                    tap_code_delay(KC_RGHT, 50);
        ...
        case KC_XXX: // X BUTTON
            ...
            if (get_mods() & MOD_MASK_CA) { // CTRL + ALT MODIFIER
                if (record->event.pressed) {
                    // EAGLE NAPALM (FIX OR SWITCHg)
                    tap_code_delay(KC_UP, 100);
                    tap_code_delay(KC_P, 50);
                    tap_code_delay(KC_RGHT, 100);
                    tap_code_delay(KC_P, 50);
